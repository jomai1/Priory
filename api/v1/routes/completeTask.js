...
router.post('/completeTask', async (req, res) => {
	...
	try{
		const task = await Task.findByIdAndUpdate(req.body._id, {status: "completed"}, { new: true, runValidators: true, strict: true });
		...
		answer.success = true;
		answer.data = task
		answer.message = "Task completed"

		res.status(200).json(answer)
	}catch(error){
		console.log("Error in completeTask")
		console.error(error);
